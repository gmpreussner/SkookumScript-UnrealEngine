...
int32_t SkUEBlueprintInterface::try_add_method_entry(UClass * ue_class_p, SkMethodBase * sk_method_p)
  {
  if (sk_method_p->get_invoke_type() == SkInvokable_method && !sk_method_p->is_operator())
    {
...
int32_t SkUEBlueprintInterface::add_event_entry(UClass * ue_class_p, SkMethodBase * sk_method_p, const FString & category)
  ...
  UFunction * ue_method_p = new(EC_InternalUseOnlyConstructor, ue_class_p, sk_method_p->get_name_cstr(), RF_Public | RF_Transient | RF_Native) UFunction(FObjectInitializer(), NULL, FUNC_BlueprintEvent | FUNC_Event | FUNC_Public, UINT16_MAX);
